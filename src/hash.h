...
namespace ByteA
{
    ...
    inline constexpr uint64_t stringHash(const char* arg)
    {
        constexpr size_t prime{ 1099511628211ULL };

        size_t value{ 14695981039346656037ULL };
        for (size_t index{}; arg[index] != '\0'; ++index) {
    ...
    inline constexpr uint64_t stringHash(const std::string& arg)
    {
        return stringHash(arg.c_str());
    }
